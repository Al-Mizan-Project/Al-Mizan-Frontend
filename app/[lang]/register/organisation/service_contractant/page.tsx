...
import { faCheck, faCloudUploadAlt, faArrowRight, faArrowLeft, faGlobe, faBuildingColumns } from '@fortawesome/free-solid-svg-icons';

interface PageParams {
  params: Promise<{ lang: string }>;
}

export default function ServiceContractantRegistration({ params }: PageParams) {
  ...
  const steps_labels = {
    1: isAr ? 'هوية المصلحة المتعاقدة' : 'Identité du Service Contractant',
    2: isAr ? 'معلومات الوصاية والجهة' : 'Tutelle et Autorité'
  };
      ...
      <div className="hidden md:flex md:w-1/3 lg:w-2/5 bg-[#005c6e] relative overflow-hidden p-12 flex-col justify-between text-white">
        <div className="z-10">
          ...
          <h2 className="text-3xl font-bold leading-tight mb-4">
            {isAr ? 'تسجيل مصلحة متعاقدة' : 'Inscription Service Contractant'}
          </h2>
          <p className="text-cyan-100 opacity-80 text-sm leading-relaxed">
            {isAr 
              ? 'بوابة الهيئات العمومية لإدارة المناقصات والمشاريع الوطنية بكل شفافية.' 
              : 'Portail des entités publiques pour la gestion des appels d\'offres et des projets nationaux.'}
          </p>
        </div>
        ...
        <FontAwesomeIcon icon={faBuildingColumns} className="absolute -bottom-10 -right-10 text-[20rem] opacity-5 -rotate-12" />
      </div>
          ...
          <form className="space-y-8" onSubmit={(e) => e.preventDefault()}>
            {currentStep === 1 && (
              <div className="grid grid-cols-1 gap-6 animate-in slide-in-from-bottom-4 duration-500">
                <FormInput name="sc_nom_officiel" label={isAr ? "الاسم الرسمي" : "Nom officiel"} placeholder="Ex: Direction des Travaux Publics" />
                <FormInput name="sc_adresse_siege" label={isAr ? "عنوان المقر" : "Adresse siège"} placeholder="..." />
                <FormInput name="sc_email_contact" label={isAr ? "البريد الإلكتروني المهني" : "Email contact"} type="email" placeholder="contact@admin.dz" />
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <FormSelect name="sc_type_entite" label={isAr ? "نوع الكيان" : "Type entité"} options={['Administration Centrale', 'Collectivité Locale', 'Etablissement Public']} />
                  <FormSelect name="sc_categorie" label={isAr ? "الفئة" : "Catégorie"} options={['Ministère', 'Wilaya', 'Commune', 'Autre']} />
                </div>
                <FormInput name="sc_code_ordonnateur" label={isAr ? "رمز الآمر بالصرف" : "Code Ordonnateur"} placeholder="Code à 10 chiffres" />
              </div>
            )}

            {currentStep === 2 && (
              <div className="grid grid-cols-1 gap-6 animate-in slide-in-from-bottom-4 duration-500">
                <FormInput name="sc_nom_tutelle" label={isAr ? "اسم الوصاية" : "Nom de la tutelle"} placeholder="Ex: Ministère de l'Intérieur" />
                <FormInput name="sc_identite_autorite" label={isAr ? "هوية السلطة" : "Identité de l'autorité"} placeholder="..." />
                
                <div className="mt-4">
                  <FileUpload 
                    name="file_acte_creation" 
                    label={isAr ? "القرار الرسمي / مرسوم الإنشاء" : "Acte officiel / Arrêté de création"} 
                  />
                </div>
                ...
                <div className="bg-amber-50 border-l-4 border-amber-400 p-4 mt-6">
                  <p className="text-xs text-amber-800 leading-relaxed">
                    {isAr 
                      ? 'يرجى التأكد من أن جميع الوثائق المرفقة هي نسخ أصلية ممسوحة ضوئياً (PDF).' 
                      : 'Veuillez vous assurer que tous les documents joints sont des scans originaux (format PDF).'}
                  </p>
                </div>
              </div>
            )}
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <label htmlFor={name} className="border-2 border-dashed border-gray-200 p-8 flex flex-col items-center justify-center hover:bg-[#005c6e]/5 hover:border-[#005c6e] cursor-pointer transition-all group min-h-[140px] rounded-sm">
        ...
        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest group-hover:text-gray-600 transition-colors">
          Téléverser le document (PDF/Image)
        </span>
      </label>
    </div>
  );
