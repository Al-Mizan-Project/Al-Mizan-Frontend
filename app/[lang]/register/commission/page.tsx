...
type FormData = {
  ...
  seuilCompetenceFinanciere: string;
  decisionCreation: File | null;
  nomCommission: string;
};

type FormErrors = Partial<Record<keyof FormData, string>>;

export default function CommissionExterneRegistration({ params }: PageProps) {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    emailContact: '',
    niveauCompetence: '',
    seuilCompetenceFinanciere: '',
    decisionCreation: null,
    nomCommission: '',
  });
  ...
  const competenceLevelOptions = [
    { value: 'communale', label: isArabic ? 'بلدية' : 'Communale' },
    { value: 'wilaya', label: isArabic ? 'ولاية' : 'Wilaya' },
    { value: 'sectorielle', label: isArabic ? 'قطاعية' : 'Sectorielle' },
    { value: 'nationale', label: isArabic ? 'وطنية' : 'Nationale' },
  ];
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.niveauCompetence) {
      newErrors.niveauCompetence = isArabic ? 'مستوى الاختصاص مطلوب' : 'Niveau de compétence requis';
    }

    if (!formData.seuilCompetenceFinanciere.trim()) {
      newErrors.seuilCompetenceFinanciere = isArabic ? 'الحد المالي مطلوب' : 'Seuil financier requis';
    } else if (isNaN(Number(formData.seuilCompetenceFinanciere))) {
      newErrors.seuilCompetenceFinanciere = isArabic ? 'يجب أن يكون رقماً' : 'Doit être un nombre';
    }

    if (!formData.decisionCreation) {
      newErrors.decisionCreation = isArabic ? 'قرار الإنشاء مطلوب' : 'Décision de création requise';
    }

    if (!formData.nomCommission.trim()) {
      newErrors.nomCommission = isArabic ? 'اسم اللجنة مطلوب' : 'Nom de la commission requis';
    }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-12 h-12 bg-[#589C9F] rounded-xl flex items-center justify-center">
              <FontAwesomeIcon icon={faScaleBalanced} className="text-white text-2xl" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-[#0D2527] font-cairo">
                {isArabic ? 'تصريح - لجنة التحقق الخارجية' : 'Déclaration - Commission de Validation Externe'}
              </h1>
              <p className="text-[#418387] font-cairo mt-1">
                {isArabic 
                  ? 'يرجى ملء النموذج أدناه لتسجيل لجنة التقييم' 
                  : 'Veuillez remplir le formulaire ci-dessous pour enregistrer votre commission d\'évaluation'
                }
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8" noValidate>
          ...
          <section className="bg-white border-2 border-[#9BCFCF] rounded-2xl p-8 shadow-sm">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <InputField
                label={isArabic ? 'الاسم الرسمي للجنة' : 'Nom officiel de la commission'}
                value={formData.nomOfficiel}
                onChange={(e) => handleChange('nomOfficiel', e.target.value)}
                error={errors.nomOfficiel}
                isArabic={isArabic}
                required
                placeholder={isArabic ? 'لجنة تقييم الصفقات العمومية' : 'Commission d\'Évaluation des Marchés Publics'}
              />

              <InputField
                label={isArabic ? 'اسم اللجنة' : 'Nom de la commission'}
                value={formData.nomCommission}
                onChange={(e) => handleChange('nomCommission', e.target.value)}
                error={errors.nomCommission}
                isArabic={isArabic}
                required
                placeholder={isArabic ? 'اللجنة القطاعية للأشغال العمومية' : 'Commission Sectorielle des Travaux Publics'}
              />
              ...
              <InputField
                ...
                isArabic={isArabic}
                required
                placeholder={isArabic ? '10 شارع الشهداء، الجزائر الوسطى' : '10 Rue des Martyrs, Alger Centre'}
              />
              ...
              <InputField
                ...
                isArabic={isArabic}
                required
                placeholder="commission@exemple.gov.dz"
                dir="ltr"
              />
            </div>
          </section>

          {/* Section 2: Competence Level */}
          <section className="bg-white border-2 border-[#9BCFCF] rounded-2xl p-8 shadow-sm">
            <h2 className="text-xl font-bold text-[#173C3F] mb-6 pb-3 border-b-2 border-[#E2E8F0] font-cairo">
              {isArabic ? 'مستوى الاختصاص' : 'Niveau de Compétence'}
            </h2>
            
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <SelectField
                label={isArabic ? 'مستوى الاختصاص' : 'Niveau de compétence'}
                options={competenceLevelOptions}
                value={formData.niveauCompetence}
                onChange={(e) => handleChange('niveauCompetence', e.target.value)}
                error={errors.niveauCompetence}
                isArabic={isArabic}
                required
                placeholder={isArabic ? 'اختر المستوى' : 'Sélectionnez le niveau'}
              />

              <InputField
                label={isArabic ? 'الحد الأدنى للاختصاص المالي (دج)' : 'Seuil de compétence financière (DA)'}
                type="number"
                value={formData.seuilCompetenceFinanciere}
                onChange={(e) => handleChange('seuilCompetenceFinanciere', e.target.value)}
                error={errors.seuilCompetenceFinanciere}
                isArabic={isArabic}
                required
          ...
          <section className="bg-white border-2 border-[#9BCFCF] rounded-2xl p-8 shadow-sm">
            ...
            <FileUpload
              label={isArabic ? 'القرار / المرسوم الإنشائي' : 'Décision/Arrêté de création'}
              helpText={isArabic ? 'صيغة PDF فقط. الحجم الأقصى: 10MB' : 'Format PDF uniquement. Taille max: 10MB'}
              error={errors.decisionCreation}
              isArabic={isArabic}
              accept=".pdf"
              maxSizeMB={10}
              onChange={(file) => handleChange('decisionCreation', file)}
            />
          </section>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-end pt-6">
            ...
            <button
              ...
              className="px-8 py-3.5 bg-[#589C9F] hover:bg-[#418387] text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all disabled:opacity-70 disabled:cursor-not-allowed shadow-lg shadow-[#589C9F]/25 font-cairo"
            >
              {loading ? (
                <>
