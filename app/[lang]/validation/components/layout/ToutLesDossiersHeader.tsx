...
interface ToutLesDossiersHeaderProps {
  lang: string;
  role: UserRole;
  dict?: {
    common?: { title?: string };
...
export default function ToutLesDossiersHeader({ lang, role, dict, dossierId }: ToutLesDossiersHeaderProps) {
  const isAr = lang === 'ar';
  const router = useRouter();
  const routes = role === 'commission' ? commissionRoutes : validatorRoutes;

  const mainTitle = role === 'commission'
