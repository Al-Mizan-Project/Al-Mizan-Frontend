...
interface AffectationHeaderProps {
  lang: string;
  role: UserRole;
  dict?: {
    common?: { title?: string };
...
export default function AffectationHeader({ lang, role, dict, dossierId }: AffectationHeaderProps) {
  const isAr = lang === 'ar';
  const router = useRouter();
  const routes = role === 'commission' ? commissionRoutes : validatorRoutes;

  const label = isAr ? 'مرجع رقم الملف' : 'REFERENCE DOSSIER ID';
  const title = isAr ? 'تعيين ملف' : "Affectation d'un Dossier";
