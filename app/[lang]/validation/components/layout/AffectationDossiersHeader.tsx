...
interface AffectationDossiersHeaderProps {
  lang: string;
  role: UserRole;
  dict?: {
    common?: { title?: string };
...
export default function AffectationDossiersHeader({ lang, role, dict, dossierId }: AffectationDossiersHeaderProps) {
  const isAr = lang === 'ar';
  const router = useRouter();
  const routes = role === 'commission' ? commissionRoutes : validatorRoutes;

  const title = isAr ? 'تعيين الملفات' : 'Affectation des Dossiers';
