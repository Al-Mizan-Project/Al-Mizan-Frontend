...
import type { EvaluateurPage } from '@/app/[lang]/evaluateur/page';

const navItems: { id: EvaluateurPage; label: string; icon: string }[] = [
  { id: 'vue-personnelle', label: 'Vue Personnelle',           icon: '⬡' },
  { id: 'mes-dossiers',   label: 'Mes Dossiers',              icon: '📁' },
  { id: 'references',     label: 'Références Réglementaires', icon: '📖' },
];

interface EvaluateurSidebarProps {
  activePage: EvaluateurPage;
  setActivePage: (id: EvaluateurPage) => void;
}

export default function EvaluateurSidebar({ activePage, setActivePage }: EvaluateurSidebarProps) {
    ...
    >
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 text-left group ${
                isActive ? 'text-white shadow-md' : 'text-gray-600 hover:bg-[#D6EAD4] hover:text-[#1C4532]'
              }`}
      ...
      <div className="px-4 py-4 border-t border-gray-100">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: '#D6EAD4' }}>
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: '#1C4532' }}>
            AB
          </div>
          <div>
            <p className="text-xs font-semibold" style={{ color: '#1C4532' }}>A. Benali</p>
            <p className="text-xs text-gray-500">Évaluateur Principal</p>
          </div>
          <button className="ml-auto text-gray-400 hover:text-[#1C4532] transition-colors text-sm">⚙</button>
        </div>
      </div>
    </aside>
  );
