...
export default function MesDossiersView() {
  ...
  const actionConfig = {
    buttons: [
      ...
      },
      {
        ...
        onClick: (d: Dossier) => console.log('[Évaluer]', d.reference),
      },
    ],
  };

  const filtered = useMemo(() => {
    return MES_DOSSIERS.filter(d => {
  ...
  if (selectedDossier) {
    return <DossierDetailView dossier={selectedDossier} onBack={() => setSelectedDossier(null)} />;
  }
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {(search || activeFilterCount > 0) && (
            <button
              onClick={() => { setSearch(''); setStatut('Tous'); setDomaine('Tous'); setPeriode('Toutes'); setCurrentPage(1); }}
        ...
        {filtersOpen && (
          <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              { label: 'Domaine',  value: domaine,  set: (v: string) => handleFilter(() => setDomaine(v)),  opts: DOMAINES },
              { label: 'Période',  value: periode,  set: (v: string) => handleFilter(() => setPeriode(v)),  opts: PERIODES },
              { label: 'Statut',   value: statut,   set: (v: string) => handleFilter(() => setStatut(v as any)), opts: STATUTS },
            ].map(f => (
              <div key={f.label}>
                ...
                <select
                  ...
                  style={{ color: '#1C4532' }}
                >
                  {f.opts.map((v: string) => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-800">Mes Dossiers</h3>
              <p className="text-xs text-gray-400">
                {filtered.length} dossier{filtered.length !== 1 ? 's' : ''} trouvé{filtered.length !== 1 ? 's' : ''}
                {filtered.length !== MES_DOSSIERS.length && (
                  <span className="text-[#00738C] font-semibold"> (filtrés sur {MES_DOSSIERS.length})</span>
                )}
              </p>
            </div>
          </div>
