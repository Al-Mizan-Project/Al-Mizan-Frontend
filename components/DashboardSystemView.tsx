...
export default function DashboardSystemView({ onNavigate }: { onNavigate: (page: 'utilisateurs' | 'roles') => void }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const actifs = users.filter((u: User) => u.is_active).length;
        const bloques = users.filter((u: User) => !u.is_active).length;
        setStats({ total, actifs, bloques });

        const roles: Record<string, number> = {};
        users.forEach((user: User) => {
          const roleName = user.id_role ? `Rôle #${user.id_role}` : 'Sans rôle';
  ...
  const StatCard = ({ label, value, sub, icon, color, onClick }: any) => (
    <div onClick={onClick} className={`bg-white border border-gray-100 rounded-2xl p-5 shadow-sm flex items-start gap-4 ${onClick ? 'cursor-pointer hover:shadow-md hover:border-gray-200 transition-all' : ''}`}>
  ...
  const STATUT_STYLE: Record<string, string> = { 'Actif': 'text-emerald-700', 'Bloqué': 'text-red-600' };
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            {Object.entries(roleCounts).map(([role, count]) => (
              <div key={role} className="flex items-center gap-3">
                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ${DEFAULT_BADGE}`}>
                  {role}
                </span>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
          ...
          <div className="space-y-3">
            {recentUsers.map((u) => (
              <div key={u.id_utilisateur} className="flex items-center gap-3">
